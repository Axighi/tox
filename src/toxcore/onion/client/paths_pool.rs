...
mod tests {
    ...
    fn self_get_path_stored() {
        ...
        assert_eq!(paths_pool.get_path(path.id(), false).unwrap(), path);
        assert_eq!(paths_pool.self_paths[0].attempts, ONION_PATH_MAX_NO_RESPONSE_USES / 2 + 1);
    }

    #[test]
    fn self_get_path_new_random() {
        crypto_init().unwrap();
        let mut paths_pool = PathsPool::new();
        for _ in 0 .. MIN_NODES_POOL_SIZE {
            let node = PackedNode::new("127.0.0.1:12345".parse().unwrap(), &gen_keypair().0);
        ...
        let path = paths_pool.get_path(path_id, false).unwrap();
    ...
    fn friends_get_path_stored() {
        ...
        assert_eq!(paths_pool.get_path(path.id(), true).unwrap(), path);
        assert_eq!(paths_pool.friend_paths[0].attempts, ONION_PATH_MAX_NO_RESPONSE_USES / 2 + 1);
    }

    #[test]
    fn friends_get_path_new_random() {
        crypto_init().unwrap();
        let mut paths_pool = PathsPool::new();
        for _ in 0 .. MIN_NODES_POOL_SIZE {
            let node = PackedNode::new("127.0.0.1:12345".parse().unwrap(), &gen_keypair().0);
        ...
        let path = paths_pool.get_path(path_id, true).unwrap();
        assert_ne!(path.id(), path_id);
        assert_eq!(path, paths_pool.friend_paths[0].path);
    }
}
