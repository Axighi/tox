...
impl OnionClient {
    ...
    pub fn handle_data_response(&self, packet: &OnionDataResponse) -> Box<Future<Item = (), Error = Error> + Send> {
        ...
        let f = match iner_payload {
            OnionDataResponseInnerPayload::DhtPkAnnounce(dht_pk_announce) => Either::A(self.handle_dht_pk_announce(payload.real_pk, dht_pk_announce)),
            OnionDataResponseInnerPayload::FriendRequest(_) => Either::B(future::ok(()))
        };
        Box::new(f)
    }
